/**
 * Shared context loader for every pace command that needs to know
 ...
 *     migrated: boolean,          // true if we auto-renamed .pace.md -> PRODUCT.md
 ...
 *   1. process.env.PACE_CONTEXT_DIR (absolute or relative to cwd)
 *   2. cwd, if PRODUCT.md / DESIGN.md / .pace.md is there (back-compat)
 *   3. Auto-fallback subdirectories of cwd: .agents/context/, then docs/
 *   4. cwd as a default "no context found" location
 *
 * Legacy `.pace.md` -> PRODUCT.md migration only fires at cwd root;
...
export function resolveContextDir(cwd = process.cwd()) {
  // 1. Explicit override
  const envDir = process.env.PACE_CONTEXT_DIR;
  ...
  //    legacy `.pace.md` does not pull the lookup into a fallback dir.
...
export function loadContext(cwd = process.cwd()) {
  ...
  // 2. Legacy: if no PRODUCT.md but .pace.md exists at cwd root, rename
