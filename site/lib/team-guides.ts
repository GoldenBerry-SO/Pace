...
export const TEAM_GUIDES: TeamGuide[] = [
    ...
    workflows: [
      {
        ...
        prompt: 'Prep me for the Acme call tomorrow.',
      },
      {
        ...
        prompt: 'Draft outreach to the Acme CFO about the pricing discussion we had last week.',
      },
      {
        ...
        prompt: 'Walk me through the Q4 pipeline. Flag stalled deals.',
      },
      {
        ...
        prompt: 'Build a battle card on Salesforce. Their positioning, common objections, how we win.',
      },
      {
        title: 'Daily briefing',
        description: 'Morning digest: calls today, deals to push, urgent Slack DMs, replies waiting.',
        prompt: 'Daily briefing. Calendar, Slack DMs, pipeline movement, what needs me first.',
      },
    ],
    tips: [
      { title: 'Run prep the night before', body: 'Schedule `call-prep` the evening before; you wake up to a brief in your inbox.' },
      ...
      { title: 'Pair with brand-voice', body: 'After `draft-outreach`, run /brand-voice:validate to catch off-tone phrases before sending.' },
      ...
      { title: 'Use account-research, not just call-prep', body: '`account-research` is research-only; `call-prep` also writes the talking points. Use the former for prospecting, the latter for booked meetings.' },
      { title: 'Trust the forecast', body: '`forecast` pulls from HubSpot stages and historical close rates. It will not flatter your numbers; that is the point.' },
    ],
    triggers: [
      { say: 'Prep me for my call with Acme tomorrow', runs: 'call-prep' },
      { say: 'Research who at Acme makes buying decisions', runs: 'account-research' },
      { say: 'Draft a follow-up to the Acme CFO about pricing', runs: 'draft-outreach' },
      { say: 'Walk me through my pipeline this week', runs: 'pipeline-review' },
      { say: 'Summarize the call I just finished with Acme', runs: 'call-summary' },
      { say: 'Compare us to Salesforce for the Acme deal', runs: 'competitive-intelligence' },
    ],
    seeAlso: [
  ...
  // ─── MARKETING ──────────────────────────────────────────────────────────
  {
    ...
    workflows: [
      {
        ...
        prompt: 'Plan the Q4 product launch campaign. Owners, channels, week-by-week.',
      },
      {
        ...
        prompt: 'Validate this blog draft against our voice guide.',
      },
      {
        title: 'Competitor brief',
        description: 'Summarizes a competitor announcement or campaign and suggests our angle of response.',
        prompt: "Brief me on Linear's launch yesterday. What changed, how it lands.",
      },
      {
        title: 'Content draft',
        ...
        prompt: 'Draft a blog post on how agentic workflows are reshaping product roadmaps.',
      },
      {
        ...
        prompt: 'Report Q3 channel performance. Where the budget actually moved the needle.',
      },
    ],
    tips: [
      ...
      { title: 'Search before writing', body: '`find` before drafting; half the "new" content already exists somewhere in Notion or Drive.' },
      { title: 'Treat the campaign plan as living', body: 'Run `campaign-plan` once at kickoff, then reference it through the campaign. Re-running mid-flight to update the plan beats wing-it.' },
      { title: 'Stack draft + validate', body: 'Always: `draft-content` → /brand-voice:validate → ship. The two-step is fast and the voice consistency compounds.' },
    ...
    triggers: [
      { say: 'Plan a launch campaign for our new pricing tier', runs: 'campaign-plan' },
      { say: 'Draft a LinkedIn post about our Q3 results', runs: 'draft-content' },
      { say: 'Pull last week\'s campaign performance', runs: 'performance-report' },
      { say: 'Check this blog draft against our brand voice', runs: 'brand-voice-enforcement' },
      { say: 'What did Notion ship last week?', runs: 'competitive-brief' },
      { say: 'Run an SEO audit on our pricing page', runs: 'seo-audit' },
    ],
    seeAlso: [
  ...
  // ─── ENGINEERING ────────────────────────────────────────────────────────
  {
    ...
    plugins: [
      ...
      { slug: 'data', role: 'companion', why: 'When reviews touch performance, SQL, or analytics; pair with the data plugin to ground in real numbers.' },
      { slug: 'productivity', role: 'companion', why: 'Standups, daily focus blocks, memory of past architecture decisions.' },
    ],
    connectors: ['(none required for engineering itself)', 'Snowflake / Databricks / BigQuery (via the data plugin)', 'Linear / Jira / Slack (via the productivity plugin)'],
    workflows: [
      {
        ...
        prompt: 'Review this PR. Look for real bugs first, then nits.',
      },
      {
        title: 'Standup summary',
        description: "Generates your async standup by reading Linear tickets touched, git commits pushed, and what's blocking.",
        prompt: "What should I say in standup? What is actually blocking me today?",
      },
      {
        ...
        prompt: 'Should we use NATS or Kafka for the new event bus? Walk me through the trade-offs.',
      },
      {
        title: 'Incident response',
        description: 'Triage playbook: diagnostic questions, likely causes, mitigation order, comms template. Useful at 3am.',
        prompt: 'P1: ingestion is lagging 40 minutes. Run an incident response.',
      },
      {
        ...
        prompt: 'Audit this directory for tech debt. What is brittle, what is stale, what should go first.',
      },
      {
        title: 'Documentation gap fix',
        description: 'Reads a module and writes the missing docs: contracts, edge cases, gotchas.',
        prompt: 'Document the rate-limiter middleware. What it does, how to tune it, gotchas.',
      },
    ],
    tips: [
      { title: 'Self-review before requesting review', body: '`code-review` before you tag a human. Catches naming and silly mistakes; the human gets to focus on intent.' },
      { title: 'Pin the deploy-checklist', body: 'Use `deploy-checklist` before any prod push. Pin it (via the Pace router pin.mjs) once your team standardizes.' },
      { title: 'Testing strategy upfront', body: '`testing-strategy` when scoping a feature, not when writing tests. Saves rewrite cycles.' },
      { title: 'Pair with /data for perf reviews', body: 'When the PR touches anything that hits the DB, run `write-query` against staging to verify the query plan before merging.' },
      { title: 'Incident response > heroics', body: '`incident-response` has saved hours at 3am. Beats inventing a playbook under pressure.' },
      { title: 'ADR convention sticks', body: "`architecture` writes ADRs in a stable format. Adopt the format and don't fight it; consistency is the point of ADRs." },
    ],
    triggers: [
      { say: 'Review this PR before I tag the team', runs: 'code-review' },
      { say: 'Catch anything I missed before I open the PR', runs: 'careful-review' },
      { say: 'Diagnose this race condition in checkout', runs: 'diagnose' },
      { say: 'Write tests for the new checkout flow using TDD', runs: 'tdd' },
      { say: 'Spec the feature so we can ship it as stacked PRs', runs: 'spec' },
      { say: 'Commit my changes and open a PR', runs: 'ship-pr' },
    ],
    seeAlso: [
    ...
    workflows: [
      {
        ...
        prompt: 'Write a SQL query for monthly active users by plan tier, last 6 months.',
      },
      {
        ...
        prompt: 'Visualize this query result. Pick the chart type that tells the story.',
      },
      {
        ...
        prompt: 'Pressure-test my conclusion that conversion improved 12% in Q3. What am I missing?',
      },
      {
        ...
        prompt: 'Run the right statistical test on these A/B results from last sprint.',
      },
      {
        title: 'Build a dashboard',
        description: 'Generates a dashboard spec (Hex, Looker, or Mode-compatible) from a set of queries.',
        prompt: 'Build a weekly product-health dashboard. Pick the metrics that matter.',
      },
      {
        ...
        prompt: 'DAU dropped 8% last Tuesday. Interpret it. Real or noise?',
      },
    ],
    tips: [
      { title: 'Always validate before sharing', body: '`validate-analysis` catches the errors that get spotted by the audience in the meeting. Cheap insurance.' },
      { title: 'Connect the warehouse first', body: "Without a connector, the data plugin writes SQL but can't run it. With one, the workflow is interactive." },
      { title: 'Document queries you keep', body: '`document-query` before saving SQL to your team library. Future-you appreciates context.' },
      { title: 'Use the three-step rhythm', body: 'For exploratory: `write-query` → `visualize` → `interpret`. Quick, surprisingly thorough.' },
      { title: 'Pair with engineering for perf', body: 'When the query is slow, hand off via `code-review` of the EXPLAIN plan.' },
      { title: 'Statistical tests are not optional for A/B', body: '`statistical-test` instead of eyeballing two means. It tells you whether the effect is real.' },
    ],
    triggers: [
      { say: 'Write a SQL query for revenue by region last quarter', runs: 'write-query' },
      { say: 'Analyze churn over the past 90 days', runs: 'analyze' },
      { say: 'Are there anomalies in our signup funnel?', runs: 'statistical-analysis' },
      { say: 'Build a dashboard for product engagement', runs: 'build-dashboard' },
      { say: 'Visualize this query result as a chart', runs: 'create-viz' },
      { say: 'Sanity-check this dataset before I share it', runs: 'validate-data' },
    ],
    seeAlso: [
  ...
  // ─── PRODUCT ────────────────────────────────────────────────────────────
  {
    ...
    intro:
      "PMs juggle synthesis: turning interviews into specs, backlog into roadmap, telemetry into stories. The mechanical part of that work is where Pace earns its keep. Run `synthesize` on a stack of interview transcripts and reclaim a day of your week.",
    plugins: [
    ...
    workflows: [
      {
        ...
        prompt: 'Spec cohort filtering for the analytics dashboard.',
      },
      {
        ...
        prompt: 'Build a Q1 roadmap from the current Linear backlog.',
      },
      {
        title: 'User research synthesis',
        description: 'Reads transcripts, surfaces patterns, names themes, generates a research summary doc.',
        prompt: 'Synthesize these 12 user interview transcripts. Themes, surprises, follow-up questions.',
      },
      {
        title: 'Stakeholder update',
        description: 'Drafts the biweekly or monthly update: what shipped, what slipped, what is next, asks.',
        prompt: 'Write the biweekly product update for the exec team.',
      },
      {
        ...
        prompt: 'Scan our space this week. Launches, fundraises, repositioning. What we should react to.',
      },
    ],
    tips: [
      { title: 'Synthesize is the killer skill', body: '`synthesize` on raw interview transcripts saves a full day every week. Connect Fireflies so transcripts arrive automatically.' },
      { title: 'Ground roadmaps in data', body: 'Pair `roadmap` with the data plugin so prioritization argues from real usage, not vibes.' },
      { title: 'Spec quality compounds', body: 'A spec that is clear, scoped, and has metrics defined upfront saves multiple weeks of re-scoping later. Invest the hour with `spec`.' },
      { title: 'Use it for retros too', body: '`synthesize` works on retro notes the same way it works on interviews. Pattern-find what teams keep saying.' },
      { title: 'Stakeholder cadence beats one-offs', body: 'Run `update` on a fixed schedule (biweekly) rather than reactively. Trust compounds with consistency.' },
      { title: 'Competitive scans weekly', body: '`competitive` once a week is enough; daily is noise.' },
    ],
    triggers: [
      { say: 'Synthesize these 8 user interviews into themes', runs: 'synthesize-research' },
      { say: 'Write a PRD for the new collaboration feature', runs: 'write-spec' },
      { say: 'Update stakeholders on this sprint\'s progress', runs: 'stakeholder-update' },
      { say: 'What did our competitors ship this week?', runs: 'competitive-brief' },
      { say: 'Brainstorm 10 ways to improve onboarding', runs: 'product-brainstorming' },
      { say: 'Plan next sprint based on this backlog', runs: 'sprint-planning' },
    ],
    seeAlso: [
    ...
    workflows: [
      {
        ...
        prompt: 'Draft journal entries from this AP batch.',
      },
      {
        title: 'Bank reconciliation',
        description: 'Reconciles the bank statement against the GL: matched, unmatched, suggested coding for outliers.',
        prompt: 'Reconcile the September bank statement against the ledger.',
      },
      {
        ...
        prompt: 'Generate the Q3 financial statements.',
      },
      {
        ...
        prompt: 'Run variance analysis on Q3 marketing spend vs budget. Tell me what to flag before the CFO asks.',
      },
      {
        title: 'Month-end close support',
        description: 'Generates the checklist, suggests order, flags items that usually slip.',
        prompt: 'Walk me through the September month-end close checklist.',
      },
      {
        title: 'Audit prep',
        ...
        prompt: 'Prep for the Y1 audit. What the auditors will ask, what we need ready.',
      },
    ],
    tips: [
      { title: 'Connect the warehouse early', body: 'Without it, the finance plugin works from pasted data. With it, queries fire directly against the GL.' },
      { title: 'Variance analysis pre-CEO', body: '`variance-analysis` surfaces the explanations before the CEO asks. Run it before every monthly close share-out.' },
      { title: 'Month-end as a workflow', body: 'Adopt `month-end` as the canonical checklist. Pin it via the Pace router once your team standardizes its close steps.' },
      { title: 'Pair with operations', body: 'For vendor and procurement work, the finance plugin + the operations plugin covers the AP → vendor lifecycle.' },
      { title: 'Audit prep is incremental', body: "Run `audit-prep` monthly, not annually. By year-end, the documentation is already there." },
      { title: 'Statements need a sanity check', body: '`generate-statements` is fast but always reconcile the output to your source-of-truth ledger before sharing externally.' },
    ],
    triggers: [
      { say: 'Run the month-end close checklist', runs: 'close-management' },
      { say: 'Explain the variance vs plan for Q3', runs: 'variance-analysis' },
      { say: 'Reconcile last month\'s bank statements', runs: 'reconciliation' },
      { say: 'Draft the journal entries for the new lease', runs: 'journal-entry-prep' },
      { say: 'Prepare audit support docs for SOX testing', runs: 'audit-support' },
      { say: 'Generate this quarter\'s financial statements', runs: 'financial-statements' },
    ],
    seeAlso: [
  ...
  // ─── LEGAL ──────────────────────────────────────────────────────────────
  {
    ...
    workflows: [
      {
        ...
        prompt: 'Review this MSA from the vendor. Flag anything unusual against our standard terms.',
      },
      {
        title: 'NDA triage',
        description: 'Compares an inbound NDA against your standard template and flags material deviations. Suggests redlines.',
        prompt: 'Compare their NDA template to ours. Where are the deltas, which matter.',
      },
      {
        ...
        prompt: 'Compliance question: can we collect EU user data this way? Walk through GDPR.',
      },
      {
        ...
        prompt: 'Risk-assess the new payment processor integration. What needs vendor diligence, what needs SOC.',
      },
      {
        ...
        prompt: 'Draft a response to this inbound enforcement query. Match our standard tone.',
      },
    ],
    tips: [
      { title: 'Connect your document store', body: 'Box or Egnyte connectors let the legal plugin read precedent docs. Without them, each review is from scratch.' },
      { title: 'Low-stakes NDAs first', body: '`nda-review` on inbound NDAs that match standard patterns is a huge time saver. Reserve human review for bespoke or high-stakes.' },
      { title: 'Compliance question ≠ legal advice', body: 'Treat `compliance-question` output as research. Final call stays with you. The skill is good at surfacing precedent and frameworks, not making the decision.' },
      { title: 'Build templated responses for repeats', body: 'After three similar inbound questions, write a `templated-response`. Saves the fourth, fifth, and sixth.' },
      { title: 'Wrap in /pace router', body: 'For your standard contract review playbook, build a /pace router command that wraps `contract-review` with your specific flags. Reuse beats re-prompting.' },
    ...
    triggers: [
      { say: 'Triage this NDA from a new vendor', runs: 'triage-nda' },
      { say: 'Review this MSA against our playbook', runs: 'review-contract' },
      { say: 'Check if our terms comply with GDPR', runs: 'compliance-check' },
      { say: 'Brief me on the upcoming board meeting', runs: 'meeting-briefing' },
      { say: 'Assess legal risk for this new partnership', runs: 'legal-risk-assessment' },
      { say: 'Run a vendor check before we sign', runs: 'vendor-check' },
    ],
    seeAlso: [
  ...
  // ─── OPERATIONS ─────────────────────────────────────────────────────────
  {
    ...
    intro:
      "Ops touches everything: vendors, processes, compliance, capacity, change management. The work is mostly about institutionalizing repeat patterns: turning tribal knowledge into docs, ad-hoc evaluations into scoring matrices, and one-off rollouts into reusable plans. the operations plugin is the systematization engine.",
    plugins: [
    ...
    workflows: [
      {
        ...
        prompt: 'Evaluate this vendor against our current stack. Capabilities, cost, integration risk.',
      },
      {
        ...
        prompt: 'Document the recurring workflow we just walked through so the next person can run it solo.',
      },
      {
        ...
        prompt: 'Plan the SSO provider migration. Phases, comms, rollback.',
      },
      {
        title: 'Capacity planning',
        description: 'Reads current team data + projected work and surfaces headcount + utilization gaps for the next quarter.',
        prompt: 'Build the quarterly headcount capacity plan. Where we are tight, where we have slack.',
      },
      {
        title: 'Compliance tracker',
        description: 'Generates and maintains a control tracker for SOC 2, ISO 27001, or similar. Flags overdue items.',
        prompt: 'Track SOC 2 controls for Q3. What is on schedule, what is overdue.',
      },
    ],
    tips: [
      { title: 'Document the tribal knowledge', body: "`document-process` is the fastest way to capture 'we should write this down' work that lives in people's heads." },
      { title: 'Vendor matrices > vibes', body: '`vendor-evaluation` generates a real scoring matrix. Beats the slide-of-bullet-points comparison every time.' },
      { title: 'Pair with the legal plugin for vendors', body: 'During vendor selection, `vendor-evaluation` → `contract-review` covers selection and contracting in one flow.' },
      { title: 'Change plans before migrations', body: '`change-plan` is cheap; rollback plans you wrote ahead of time are priceless.' },
      { title: 'Tie capacity to /data', body: 'For honest capacity planning, ground in actual utilization. Pair `capacity` with the data plugin queries on time tracking or work logs.' },
      { title: 'Compliance is incremental', body: '`compliance-tracker` monthly cadence keeps audit prep from being a fire drill.' },
    ],
    triggers: [
      { say: 'Pull together this week\'s cross-team status', runs: 'status-report' },
      { say: 'Document the incident response runbook', runs: 'runbook' },
      { say: 'Review this new vendor before we sign', runs: 'vendor-review' },
      { say: 'Plan capacity for next quarter', runs: 'capacity-plan' },
      { say: 'Assess the risks of moving to this new provider', runs: 'risk-assessment' },
      { say: 'Optimize the way we ship hardware to new hires', runs: 'process-optimization' },
    ],
    seeAlso: [
    ...
    workflows: [
      {
        ...
        prompt: 'Write a job description for a senior backend engineer, band 3.',
      },
      {
        title: 'Onboarding plan',
        description: 'Generates a personalized 30/60/90-day plan by role. Includes intros, reading list, first projects.',
        prompt: 'Build the first-30-days onboarding plan for the new hire.',
      },
      {
        ...
        prompt: 'Consolidate the 360-review inputs for this person into a coherent summary.',
      },
      {
        title: 'Compensation analysis',
        description: 'Compares a band against market data, flags compression risks, suggests adjustments.',
        prompt: 'Run a comp analysis on the band-3 engineering population. Market median, internal distribution, outliers.',
      },
      {
        title: 'Policy lookup',
        description: 'Answers a policy question by reading the connected docs.',
        prompt: "What is our parental leave policy? Pull the current one from the handbook.",
      },
    ],
    tips: [
      { title: 'Onboarding plans personalize', body: '`onboarding-plan` adapts by role. Hours saved per hire compound across teams.' },
      ...
      { title: 'Use review-feedback for patterns', body: 'Eyeballing 360s misses patterns; `review-feedback` surfaces them.' },
      { title: 'Comp analysis isn\'t a decision', body: 'Treat the output as a starting point for the comp conversation, not the answer.' },
      { title: 'Pair with /enterprise-search', body: 'For "did we already write something about X?" questions, the search plugin is faster than scrolling Slack.' },
      { title: 'JDs as a template family', body: "After three JDs for similar roles, build a /pace router command that wraps `job-description` with your leveling defaults." },
    ],
    triggers: [
      { say: 'Walk me through this week\'s recruiting funnel', runs: 'recruiting-pipeline' },
      { say: 'Prep me to interview Jane for the staff role', runs: 'interview-prep' },
      { say: 'Draft the offer letter for Jane at L5', runs: 'draft-offer' },
      { say: 'Help me write Maria\'s performance review', runs: 'performance-review' },
      { say: 'Plan headcount for the engineering org next quarter', runs: 'org-planning' },
      { say: 'Look up our PTO policy for new parents', runs: 'policy-lookup' },
    ],
    seeAlso: [
    ...
    workflows: [
      {
        ...
        prompt: 'Triage the Intercom queue from the last 24 hours. Group by theme, flag the urgent ones.',
      },
      {
        ...
        prompt: 'Draft a response for ticket #1234.',
      },
      {
        ...
        prompt: 'Escalate ticket #1234 to engineering with the context they need.',
      },
      {
        ...
        prompt: 'Pull full context for this customer. Open tickets, recent activity, deal status if any.',
      },
      {
        ...
        prompt: 'Turn resolved ticket #1234 into a knowledge-base article.',
      },
    ],
    tips: [
      ...
      { title: 'Resolved tickets are KB gold', body: '`knowledge-base` auto-converts. Run it after every interesting resolution; the next ticket finds the answer.' },
      { title: 'Escalations save dev time', body: '`escalate` writes proper repro steps. Engineering thanks you.' },
      { title: 'Stack with /sales for accounts', body: "For VIP accounts, `context` + the sales plugin gives you the full deal + product-usage picture in one brief." },
      { title: 'Build /pace router for escalation paths', body: 'Your escalation lanes are unique to your team. Wrap `escalate` in /pace router commands for each lane.' },
      { title: 'Run triage at the start of every shift', body: '`triage` gives you a sorted queue + visibility to P1s that need air cover.' },
    ],
    triggers: [
      { say: 'Triage the support inbox', runs: 'ticket-triage' },
      { say: 'Draft a response to this Intercom ticket', runs: 'draft-response' },
      { say: 'Should I escalate this customer issue?', runs: 'customer-escalation' },
      { say: 'Write a KB article for this common question', runs: 'kb-article' },
      { say: 'Research what this customer\'s account looks like', runs: 'customer-research' },
    ],
    seeAlso: [
    ...
    workflows: [
      {
        ...
        prompt: 'Start my day. Calendar, Slack DMs, Linear, what should I touch first.',
      },
      {
        title: 'Async update',
        description: 'Summarizes what changed since yesterday across the tools you use. Posts as standup or async update.',
        prompt: 'What changed in Linear since yesterday? Show me the deltas.',
      },
      {
        ...
        prompt: 'Remind me what we decided about pricing last sprint.',
      },
      {
        title: 'Find something',
        description: 'One query across every connected tool. Better than scrolling Slack hoping to find that link.',
        prompt: 'Find the Q3 launch retro. I know we wrote it, I cannot remember where.',
      },
      {
        title: 'Task management',
        description: 'Cleans up your task list, surfaces stale items, suggests prioritization.',
        prompt: 'What are my actual tasks for this week? Pull from Linear and the calendar.',
      },
    ],
    tips: [
      { title: '`start` every morning', body: 'Replaces three tab-switches with one prompt. Habit-forming.' },
      { title: 'Memory compounds', body: 'The more you use `memory`, the better it gets at finding past context.' },
      { title: 'Enterprise-search beats Slack search', body: 'Slack search is bad. `find` is the "where did we put X?" answer machine.' },
      { title: 'Connect Slack first', body: 'It unlocks the highest-volume tools. Authorize once; both the productivity plugin and the search plugin benefit.' },
      ...
      { title: 'Async updates beat live standups', body: '`update` for async standups frees the team from a daily live meeting.' },
    ],
    triggers: [
      { say: 'What\'s on my plate this week?', runs: 'task-management' },
      { say: 'Write my end-of-day update', runs: 'update' },
      { say: 'Pick up where I left off yesterday', runs: 'start' },
      { say: 'Remember that we decided X about Y', runs: 'memory-management' },
    ],
    seeAlso: [
      { slug: 'sales', label: 'Sales', why: 'Pace with the productivity plugin + sales is the GTM stack.' },
