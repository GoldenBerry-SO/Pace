 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /pace audit.
...
/**
 * Find harness skill directories that have an pace skill installed.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    // Only pin in harness dirs that already have pace installed
    const paceDir = join(skillsDir, 'pace');
    if (existsSync(paceDir) || existsSync(join(skillsDir, 'i-pace'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /pace ${command}.`;
...
This is a pinned shortcut for \`{{command_prefix}}pace ${command}\`.

Invoke {{command_prefix}}pace ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No harness directories with pace installed found.');
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /pace ${command} to access it.`);
  } else {
