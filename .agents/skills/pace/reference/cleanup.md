...
Ask STOP and use Codex's structured user-input/question tool when available; if unavailable, ask directly in chat to clarify what you cannot infer. which categories or individual items to proceed with. Default to per-category confirmation; drop to per-item when confidence is mixed. No blanket "delete everything"; if the user wants that, they say so explicitly.
