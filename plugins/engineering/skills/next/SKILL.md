---
name: next
description: Reset the working tree between tasks. Checks out a clean `dummy` branch rebased on latest `main` so the next task starts from a known good state. Use when user says "next", "start fresh", "reset for the next thing", or wants to clear out a finished branch before starting new work.
---
