...
//   New installs write project config at .pace/live/config.json instead.
...
/**
 * Read and parse the pace skill source.
 ...
 *
 * Defaults to the main pace SKILL.md but accepts any relative path so
...
const EXCLUDED_FROM_SUGGESTIONS = new Set([
  'pace',               // foundational skill, not a steering command
  'teach-pace',         // deprecated shim
...
export function replacePlaceholders(content, provider, commandNames = [], allSkillNames = []) {
  ...
  // After the v3.0 consolidation, commands are sub-commands of /pace.
  // If there's only one user-invocable skill (pace), generate sub-command references.
  ...
  if (nonExcluded.length === 0) {
    // Single-skill architecture: list sub-commands as /pace <sub>
    commandList = PACE_SUB_COMMANDS
      .map(n => `${cmdPrefix}pace ${n}`)
      .join(', ');
  } else {
