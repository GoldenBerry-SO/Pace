...
const EXCLUDED_SKILLS = new Set([
  'frontend-design',   // deprecated, renamed to pace
  'teach-pace',  // deprecated, folded into /pace teach
...
export async function buildSubPageData(rootDir) {
  ...
  // After the v3.0 consolidation there's only one source skill (pace).
  ...
  const paceSkill = rawSkills.find((s) => s.name === 'pace');
  ...
  const allCommandNames = ['pace', ...Object.keys(commandMetadata)];
  const resolvePlaceholders = (content) =>
    replacePlaceholders(content, 'claude-code', [], allCommandNames);

  const skills = [];

  // 1. The root pace skill itself.
  if (paceSkill && !EXCLUDED_SKILLS.has(paceSkill.name)) {
    const editorial = readEditorialWrapper(contentDir, 'skills', 'pace');
    const demo = commandDemos['pace'] || null;
    skills.push({
      id: 'pace',
      name: 'pace',
      description: paceSkill.description,
      argumentHint: paceSkill.argumentHint,
      category: SKILL_CATEGORIES['pace'],
      body: resolvePlaceholders(paceSkill.body),
      references: (paceSkill.references || []).map((r) => ({
        ...r,
  ...
  if (paceSkill) {
    for (const [cmdId, meta] of Object.entries(commandMetadata)) {
      ...
      const refFile = paceSkill.references?.find((r) => r.name === cmdId);
