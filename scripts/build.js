...
// Pace does not ship an anti-pattern detector. Impeccable owns that. Stub
// ANTIPATTERNS as empty so the build's count-generation logic still runs
// without referencing a deleted file.
const ANTIPATTERNS = [];
...
function generateCounts(rootDir, skills, buildDir) {
  ...
  // of /pace. Count them from the command router table in SKILL.md.
  const paceSkill = skills.find(s => s.name === 'pace');
  let commandCount;
  if (paceSkill) {
    ...
    const routerMatches = paceSkill.body.match(/^\| `[^`]+` \|/gm);
...
/**
 * Narrow prose check for the pace skill source.
 *
...
function generateApiData(buildDir, skills, patterns) {
  ...
  // /pace. Load them from command-metadata.json and include the root
  // pace skill itself so UI surfaces like the cheatsheet can list them.
  ...
  const pace = skills.find(s => s.name === 'pace');
  if (!pace) {
    throw new Error('pace skill not found at skill/SKILL.md. The build system expects exactly one skill at that path.');
  }
...
async function build() {
  ...
  const deprecatedLocalSkills = [
    'frontend-design', 'teach-pace',
    'arrange', 'normalize', 'onboard', 'extract',
    // v3.0 consolidation: standalone skills -> /pace sub-commands
  ...
  const claudeSkillsSrc = path.join(DIST_DIR, 'claude-code', '.claude', 'skills', 'pace');
  if (fs.existsSync(claudeSkillsSrc)) {
    fs.mkdirSync(pluginSkillsDir, { recursive: true });
    copyDirSync(claudeSkillsSrc, path.join(pluginSkillsDir, 'pace'));
  }
