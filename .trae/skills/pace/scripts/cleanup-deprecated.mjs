#!/usr/bin/env node
/**
 * Cleans up deprecated Pace skill files, symlinks, and
 ...
 *      "pace" (to avoid deleting unrelated user skills).
...
const DEPRECATED_NAMES = [
  // v2.0 renames
  'frontend-design',    // renamed to pace
  'teach-pace',   // folded into /pace teach
  // v2.1 merges
  'arrange',            // renamed to layout
  'normalize',          // merged into polish
  'onboard',            // merged into harden
  'extract',            // merged into /pace extract
  // v3.0 consolidation: all standalone skills -> /pace sub-commands
  'adapt',
  'animate',
  'audit',
  'bolder',
...
// "pace" in their v2.x source. Used as a last-resort match
...
/**
 * Check whether a skill directory belongs to Pace. Three layered
 * signals, in order of reliability:
 *   1. Lock source equals "GoldenBerry-SO/pace.tools" (authoritative).
 *   2. SKILL.md body contains the word "pace".
 ...
 */
export function isPaceSkill(skillDir, { skillName, lock } = {}) {
  // 1. Authoritative: the lock file claims this skill is ours.
  if (skillName && lock?.skills?.[skillName]?.source === 'GoldenBerry-SO/pace.tools') {
    return true;
  }
  ...
  if (/pace/i.test(content)) return true;
 ...
 * drive deletion even when SKILL.md never mentions pace.
    ...
    for (const name of targets) {
      ...
      if (stat.isSymbolicLink()) {
        ...
        const isMatch = targetAlive
          ? isPaceSkill(skillPath, { skillName: name, lock })
          : true;
        if (isMatch) {
      ...
      // Regular directory -- verify it belongs to pace
      if (isPaceSkill(skillPath, { skillName: name, lock })) {
 ...
 * Only removes entries whose source is "GoldenBerry-SO/pace.tools".
...
export function cleanSkillsLock(projectRoot) {
  ...
  for (const name of targets) {
    ...
    if (!entry) continue;
    // Only remove if it belongs to pace
    if (entry.source === 'GoldenBerry-SO/pace.tools') {
...
if (process.argv[1] && resolve(process.argv[1]) === resolve(new URL(import.meta.url).pathname)) {
  ...
  if (result.deletedPaths.length === 0 && result.removedLockEntries.length === 0) {
    console.log('No deprecated Pace skills found. Nothing to clean up.');
  } else {
