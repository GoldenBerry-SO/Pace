...
Everyone in a company who uses an AI coding agent (Claude Code, Cursor, Codex CLI, Gemini CLI, and others) and wants a shared, repeatable way to do work. Engineers running PR reviews. PMs writing specs. Ops folks on runbooks. Marketing on launch copy. Sales people prepping calls. Finance closing the month. Whatever department, whatever workflow.

They land on `pace.tools` from internal docs, Slack, or word of mouth. They already know what skills are; they want a curated set that matches how your company works.
