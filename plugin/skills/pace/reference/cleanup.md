...
Ask STOP and call the AskUserQuestion tool to clarify. which categories or individual items to proceed with. Default to per-category confirmation; drop to per-item when confidence is mixed. No blanket "delete everything"; if Chris wants that, he says so explicitly.
