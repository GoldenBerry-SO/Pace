...
```bash
node .cursor/skills/pace/scripts/load-context.mjs
```
...
Users can promote any pace command to a standalone top-level slash command with `node .cursor/skills/pace/scripts/pin.mjs <command>`. This creates a thin redirect so `/audit` (for example) delegates to `/pace audit`. `pin.mjs --unpin <command>` removes the shim.
